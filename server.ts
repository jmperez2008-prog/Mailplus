...
async function startServer() {
  ...
  app.post("/api/login", async (req, res) => {
    ...
    if (await bcrypt.compare(password, user.password)) {
      ...
      // Normalize keys for frontend (snake_case DB -> camelCase frontend if needed, but let's keep it simple)
      // We might need to map smtp_config to smtpConfig if we want to be consistent
      const mappedUser = {
        ...userWithoutPassword,
